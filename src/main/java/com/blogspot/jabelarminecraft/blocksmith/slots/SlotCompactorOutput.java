 ...
 *
 */
public class SlotCompactorOutput  extends Slot
{
    /** The player that is using the GUI where this slot resides. */
    private final EntityPlayer thePlayer;
    private int numCompactorOutput;
	...
	public ItemStack decrStackSize(int parAmount)
    {
        if (getHasStack())
        {
            numCompactorOutput += Math.min(parAmount, getStack().stackSize);
        }
    ...
    {
        numCompactorOutput += parAmountGround;
...
//        parItemStack.onCrafting(thePlayer.worldObj, thePlayer, field_75228_b);
        ...
        {
            int expEarned = numCompactorOutput;
